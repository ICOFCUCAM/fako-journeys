...
**BUILT AND LIVE.** This was written as a proposal; the decisions were settled
and the shell was implemented across all 1,597 pages. It is kept as the
specification the implementation is measured against.

> **THIS FILE IS NOT A PAGE.** It is one of 38 documents in `docs/`, none of
> which is served: `vercel.json` maps nothing there and there is no HTML in it.
> Looking at this file on GitHub shows you the plan. The navigation it describes
> is in the built HTML — `<a class="af-shell-area" href="/places">Explore</a>`
> on every page — and that is the thing a visitor sees.

The original note read: *Instruction 18 is explicit: the final
...
──────────────────────────────────────────────────────────
About ......... /about-afrinkong   BUILT. Generated by trust.py
...
## The seven questions — all settled

Answered, and built. Recorded with the reasoning because the reasoning is what a
future change has to argue against.

| | question | decision |
|---|---|---|
| 1 | Journey Fund — PLAN or FUND? | **PLAN.** It is a planner that holds nothing; FUND becomes a first-class area when the financial product is legally and operationally live |
| 2 | "Travel Goal" or "My Goals"? | **Travel Goal.** "My Goals" implies an account and there are none |
| 3 | "Every place" or "Destinations"? | **Destinations.** Clearer internationally, and it scales |
| 4 | TRAVEL in the navigation? | **No, not yet.** Bookable travel is not operational, and an item that says "press here to do a thing you cannot do" is the failure this programme exists to avoid |
| 5 | An "About Afrinkong"? | **Built, at `/about-afrinkong`.** Explicit, readable, and it leaves `/about` — the operator's — where it is |
| 6 | Trans Afrique's masthead | **Product band beneath the platform bar.** One shell, identity on top of it |
| 7 | ⌘K | No action. It becomes a convenience rather than the architecture |

The original framing of each is kept below, because the argument matters more
than the answer.
